...
function get_cats(){
	...
	$sql = "select * from #_user_group where id!=1";		
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id, photo from #_user where id_item='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_sanpham.$row['photo']);
				$sql = "delete from #_user where id_item='".$id."'";
			}
			$d->query($sql);
		}
...
function get_items(){
	...
	$sql = "select * from #_user where id!=1 order by username";
	$d->query($sql);
	$items = $d->result_array();
...
function save_item(){
	...
	if($id){ // cap nhat
		...
		if($d->num_rows()>0){
			$row = $d->fetch_array();
			if($row['id'] == 1) transfer("Bạn không có quyền trên tài khoản này.<br>Mọi thắc mắc xin liên hệ quản trị website.", "index.php?com=user&act=man");
		}
...
function login(){
	...
	transfer("Tên đăng nhập, mật khẩu không chính xác", "login.php");
}
