...
function getUnit($unit){
    ...
    elseif($unit == 'chai500ml') {
        return 'Chai 500ml';
    }
    elseif($unit == 'chai250ml') {
        return 'Chai 250ml';
    }
    elseif($unit == 'goi200g') {
        return 'Gói 200gr';
...
function paging($r, $url='', $curPg=1, $mxR=5, $maxP=5, $class_paging='')
{
	if($curPg<1) $curPg=1;
	if($mxR<1) $mxR=5;
	if($maxP<1) $maxP=5;
	...
	if($totalRows>$mxR){
		...
		for($j = $from; $j <= $to; $j++) {
			if ($j == $curPg){
				...
				$paging1 .= " <a class='paging transitionAll' href='".$url."&curPage=".$j."'>".$j."</a> ";
			}
		} //for
		$paging .=" <a href='".$url."'  style='display: inline-block; font-size: 20px; line-height: 11px;' >&laquo;</a> "; //ve dau

		$paging .=" <a href='".$url."&curPage=".($curPg-1)."' style='display: inline-block; font-size: 20px; line-height: 11px; ' >&#8249;</a> "; //ve truoc

		$paging.=$paging1;

		$paging .=" <a href='".$url."&curPage=".($curPg+1)."' style='display: inline-block; font-size: 20px; line-height: 11px;' >&#8250;</a> "; //ke

		$paging .=" <a href='".$url."&curPage=".($totalPages)."' style='display: inline-block; font-size: 20px; line-height: 11px;' >&raquo;</a> "; //ve cuoi
	}
	$r3['curPage']=$curPg;
	$r3['source']=$r2;
	$r3['paging']=$paging;
...
$value = explode('.', $image_url);
$image_ext = trim(strtolower(end($value)));
...
// scale down and add borders
	...
	$color = imagecolorallocatealpha ($canvas, 255, 255, 255, 1);
...
function getcat($id,$id_paren='0',$catData, $prentsID=0,$gach='') {
   $s='';
   if (array_key_exists($prentsID, $catData)) {
      foreach($catData[$prentsID] as $subID => $subName) {
		 if($subID==$id_paren) $s='selected'; else $s='';
         if($subID!=$id) {
			 $catList .= '<option value="'.$subID.'" '.$s.' >'.$gach.$subName.'</option>';
			 $catList .= getcat($id,$id_paren,$catData, $subID,'--'.$gach);
		 }
      }
   }
   return $catList;
...
function getcompareAdmin($catData, $prentsID=0, $properties) {
   $s='';
   if (array_key_exists($prentsID, $catData)) {
      foreach($catData[$prentsID] as $subID => $subName) {

		 for($i=0;$i<count($properties['noidung']);$i++){

			if($properties['id_compare'][$i]==$subID){//echo $properties['id_compare'][$i].'-'.$subID.'<br/>';
			break;
			}
		 }

		 if($prentsID==0){
			$catList .= '<div class="panel-heading"><p style="text-align: center;font-weight: bold;">'.$subName.'</p></div>';
		 }else{
			$catList .= '<div class="form-group row"><div class="col-lg-2">'.$subName.'</div><div class="col-lg-8"><input type="hidden" name="id_compare[]" value="'.$subID.'" /><textarea name="noidung[]" rows="3" class="form-control" >'.$properties['noidung'][$i].'</textarea></div></div>';
		 }
		 $catList .= getcompareAdmin($catData, $subID, $properties);
...
function getcompare($catData, $prentsID=0, $properties) {
   $s='';
   if (array_key_exists($prentsID, $catData)) {
      foreach($catData[$prentsID] as $subID => $subName) {
		 ...
		 if($prentsID==0){
			$catList .= '<tr><td colspan="2" style="width:10%;text-align:left;padding: 5px;background: #CCC;"><strong>'.$subName.'</strong></td></tr>';
		 }else{
			$catList .= '<tr><td style="width:10%;text-align:left;padding: 5px;">'.$subName.' <input type="hidden" name="id_compare[]" value="'.$subID.'"></td><td><textarea name="noidung[]" style="width: 90%; height: 50px;">'.$properties['noidung'][$i].'</textarea></td></tr>';
		 }
		 $catList .= getcompare($catData, $subID, $properties);
...
function getWidget($catData, $prentsID=0) {
   ...
   if (array_key_exists($prentsID, $catData)) {
      foreach($catData[$prentsID] as $subID => $subName) {
		 ...
		 if($prentsID!=0){
			if($img['photo']){
			$txt = '<img src="'._upload_sanpham_l.$img['photo'].'" align="absmiddle" width="85px" height="25px" style="margin-right:10px">';
			}else{
			$txt ='';
			}

         	$catList .= '<li><a href="#" class="transitionAll" rel="'.$subID.'">'.$txt.$subName.'</a></li>';
		 }else{
			$catList .= '<div class="optionBox"><h3>'.$subName.'</h3><h6>('._option.')</h6><div class="reviewSelect"><img src="'._upload_sanpham_l.$img['photo'].'" width="70" height="70" style="position: absolute;bottom:0px;right: -90px;"><input type="hidden" name="widget" value="0"><a href="#" class="selected transitionAll">Not Required</a><ul class="reviewSelect_UL"><li><a href="#" class="transitionAll" rel="0" >Not Required</a></li>';
		 }
         $catList .= getWidget($catData, $subID);
...
function getidcatinparent($cap = 0,$arr = NULL){
	if(!$arr) $arr = array();//khoi tao 1 array co ten la arr
	$arr[]=$cap;
	$sql = mysql_query("SELECT id FROM table_product_widget WHERE id_parents = ".$cap);
	while($row = mysql_fetch_assoc($sql)){
	$arr[]= $row['id'];
	$arr = getidcatinparent($row['id'],$arr);
	}
	return $arr;
}
